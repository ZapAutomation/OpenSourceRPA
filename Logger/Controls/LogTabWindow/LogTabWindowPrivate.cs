...
namespace ZappyLogger.Controls.LogTabWindow
{
    public partial class LogTabWindow
    {
        ...
        private void SaveLastOpenFilesList()
        {
            ...
            {
                if (content is LogWindow.LogWindow)
                {
                    LogWindow.LogWindow logWin = content as LogWindow.LogWindow;
        ...
        private void ChangeCurrentLogWindow(LogWindow.LogWindow newLogWindow)
        {
            ...
            LogWindow.LogWindow oldLogWindow = currentLogWindow;
            currentLogWindow = newLogWindow;
            string titleName = showInstanceNumbers ? "ZappyLogger #" + instanceNumber : "ZappyLogger";
                ...
                if (DialogResult.Cancel != dlg.ShowDialog())
                {
                    switch (dlg.ProjectLoadResult)
                    {
                        ...
                        case ProjectLoadDlgResult.NewWindow:
                            ZappyLoggerProxy.NewWindow(new string[] { projectFileName });
                            return;
                    }
                }
            }
