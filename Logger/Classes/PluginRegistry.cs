...
using ZappyLogger.Classes.Columnizer;
...
namespace ZappyLogger.Classes
{
    ...
    internal class PluginRegistry
    {
        ...
        private readonly IList<IZappyLoggerPlugin> pluginList = new List<IZappyLoggerPlugin>();
                                ...
                                if (type.Name.EndsWith("Columnizer"))
                                {
                                    ...
                                    if (inter != null)
                                    {
                                        ...
                                        if (cti != null)
                                        {
                                            ...
                                            if (o is IZappyLoggerPlugin)
                                            {
                                                this.pluginList.Add(o as IZappyLoggerPlugin);
                                                (o as IZappyLoggerPlugin).PluginLoaded();
                                            }
        ...
        {
            foreach (IZappyLoggerPlugin plugin in this.pluginList)
            {
                plugin.AppExiting();
            }
        }
        ...
        private bool TryAsContextMenu(Type type)
        {
            ...
            if (me != null)
            {
                ...
                if (me is IZappyLoggerPluginConfigurator)
                {
                    ((IZappyLoggerPluginConfigurator)me).LoadConfig(ConfigManager.ConfigDir);
                }
                if (me is IZappyLoggerPlugin)
                {
                    this.pluginList.Add(me as IZappyLoggerPlugin);
                    (me as IZappyLoggerPlugin).PluginLoaded();
                }
        ...
        private bool TryAsKeywordAction(Type type)
        {
            ...
            if (ka != null)
            {
                ...
                if (ka is IZappyLoggerPluginConfigurator)
                {
                    ((IZappyLoggerPluginConfigurator)ka).LoadConfig(ConfigManager.ConfigDir);
                }
                if (ka is IZappyLoggerPlugin)
                {
                    this.pluginList.Add(ka as IZappyLoggerPlugin);
                    (ka as IZappyLoggerPlugin).PluginLoaded();
                }
        ...
        private bool TryAsFileSystem(Type type)
        {
            ...
            if (fs != null)
            {
                ...
                if (fs is IZappyLoggerPluginConfigurator)
                {
                    ((IZappyLoggerPluginConfigurator)fs).LoadConfig(ConfigManager.ConfigDir);
                }
                if (fs is IZappyLoggerPlugin)
                {
                    this.pluginList.Add(fs as IZappyLoggerPlugin);
                    (fs as IZappyLoggerPlugin).PluginLoaded();
                }
